...
import { formatJson, formatMessages, formatMessagesMarkdown, getOutputFormat } from '../utils/formatter.js';
import ora from 'ora';
...
export const readCommand = new Command('read')
  ...
  .action(async (chat: string, options: { limit?: string; since?: string; until?: string; json?: boolean; markdown?: boolean }) => {
    const spinner = ora(`Fetching messages from "${chat}"...`).start();
    try {
      const client = await getClient();
      const fetchOptions: { limit: number; minDate?: Date; maxDate?: Date } = {
